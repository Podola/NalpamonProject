    ...
    public override void OnInspectorGUI()
    {
        ...
        if (manager.playlist != null)
        {
            for (int i = 0; i < manager.playlist.Count; i++)
            {
                ...
                {
                    if (item.sceneMapping == null)
                    {
                        item.sceneMapping = new SceneTimelineMapping();
                    }
                    item.sceneMapping.sceneName = EditorGUILayout.TextField("Scene Name", item.sceneMapping.sceneName);
                    item.sceneMapping.timelineAsset = (TimelineAsset)EditorGUILayout.ObjectField("Timeline Asset", item.sceneMapping.timelineAsset, typeof(TimelineAsset), false);
                    // id 자동 동기화: 타임라인 타입이면 id를 TimelineAsset의 이름으로 설정
                    if (item.sceneMapping.timelineAsset != null)
                    {
                        string autoID = item.sceneMapping.timelineAsset.name;
